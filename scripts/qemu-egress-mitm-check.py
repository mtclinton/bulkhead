...
try:
    ...
    # ===== ARM 5 (ADR-0034 inc2 sub-B): the SOUND HTTP-method allowlist for inspected egress. The first
    #       attempt was REVERTED (a one-shot first-line check was bypassable by HTTP/1.1 pipelining); the
    #       inspect leg is now HTTP/1.1-AWARE — every request is PARSED + re-FRAMED by the proxy — so the
    #       rule holds. With INSPECT_METHODS=POST the agent's GET to the inspect host is DENIED before any
    #       body relays, signed as a Hook=inspect deny reason=method:GET. (The pipelining bypass itself is
    #       closed by construction and unit-proven in TestInspectRelayPipelineBypass.) =====
    ...
    sout6, jr6 = run_agent("mitmmethods")
    ...
    check('"decision":"deny"' in methrec and "reason=method:GET" in methrec,
          "[methods] the inspect record is a DENY with reason=method:GET (the disallowed method, recorded honestly)")
