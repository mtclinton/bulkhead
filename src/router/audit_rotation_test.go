...
// the router's segments by this exact "%s.%06d" pattern; any drift here silently makes verify-audit miss
// or misread the router segments, so this fails CI first (mirrors the canonical() golden discipline).
...
func rotAppendN(t *testing.T, a *auditLog, n int) {
	t.Helper()
	for i := 0; i < n; i++ {
		if err := a.append(auditEvent{Comm: "router", Hook: "route", Decision: "local", Mode: "reason=x model=m promptlen=5"}); err != nil {
...
func TestRotationBoundsFootprint(t *testing.T) {
	...
	a, err := openAuditLog("router", "provenance.jsonl")
	if err != nil {
		t.Fatal(err)
	}
	rotAppendN(t, a, 200)
...
// TestRotationRenameFailureContinuesAppend is the R1 invariant for the router chain: a blocked rotation
...
func TestRotationRenameFailureContinuesAppend(t *testing.T) {
	...
	for i := 0; i < 40; i++ {
		if err := a.append(auditEvent{Comm: "router", Hook: "route", Decision: "local", Mode: "reason=x"}); err != nil {
