...
Relates to: ADR-0032 (interception primitive), ADR-0033 (io_uring broker), ADR-0034 (egress), ADR-0035 (action authorization), ADR-0037 (multi-agent domains reuse this substrate), ADR-0038 (confidential computing rejected); refines the agent-isolation pillar of ADR-0001.
...
Two sub-questions must be answered together: (1) what *kind* of boundary do we put under the default agent tier, and (2) do we build that boundary ourselves or reuse a hardened artifact. The bulkhead codebase carries a mini-sentry and a mini-firecracker prototype, so the build-vs-reuse tension is live, not hypothetical.
...
Bulkhead **will** make a separate-kernel boundary mandatory for the default and hostile agent tiers. The **default** isolation boundary is a reimplemented-kernel Sentry (gVisor-class), continuing the mini-sentry lineage as the shippable default. The **hostile tier** runs inside a **reused Firecracker microVM** (Cloud Hypervisor where more device breadth is required). Bulkhead **will not** ship a hand-rolled C VMM as any tier's boundary. Namespace+seccomp is permitted **only** for an explicitly trusted, low-threat tier — never as the default. The mini-firecracker C work is retained as a learning artifact and for control-plane use, not as the hostile-tier guest boundary.
...
- Host syscall surface for the default tier collapses ~350→~68 [#1]; a Sentry breach is not by itself a host escape [#10].
