...
5. **(in-image, build-verified)** the firecracker Yocto recipe (`firecracker_1.15.1.bb`, the runsc prebuilt-binary model — pinned static firecracker + jailer, sha256-checked) + `bulkhead-fc-vsockmux_0.1.0.bb` (the mux, native-cross Go), both confirmed as ELF in the image rootfs; `bulkhead-fc-guestkernel.bb` extracts the ELF vmlinux from the deploy bzImage at build (firecracker rejects the gzip'd bzImage) → `/usr/share/bulkhead/fc/vmlinux`. The FC-tuned guest kernel (io_uring off per ADR-0033; drop VIRTIO_NET) is a follow-up — the no-NIC guarantee already rests on omitting the firecracker net stanza, not the kernel. `e2fsprogs-mke2fs` made an explicit image dep (the launcher's runtime rootfs build).
6. **(deployable, build-verified; microVM boot bare-metal-pending)** `bulkhead-agent-firecracker-launch` (the `[A-Za-z0-9_-]` inst guard; mirrors the live-proven `fc-agent-check.sh` rootfs assembly — busybox is a dynamic PIE and the minimal image has no ldd/readelf, so its loader + NEEDED glibc libs copy by ABI-stable name) + `bulkhead-agent-firecracker@.service` (Requires/After the mux + proxy + selftest gate) + `bulkhead-fc-vsockmux@.service` (the per-instance host mux at the tightest hardening floor). The mechanism is live-proven host-side (slices 1–4); the qemu suite has no nested KVM, so the boot path is build-verified only and needs a bare-metal (root + `/dev/kvm`) env. REMAINING FOLLOW-UPS there: wrap firecracker under the jailer (its chroot must coordinate the mux's leg dir — the one un-validated cross-process socket layout), a two-instance arm asserting cross-instance isolation, and per-instance vsock-CID allocation (`BULKHEAD_FC_CID`, default 3, is single-instance for now).
